...
# optional overrides:
# namespace        = "rhel-10-manitoba-auto"
...
The output prints the VM's cluster DNS, e.g. `rhel-10-demo-vm-terraform.headless-tf.rhel-10-manitoba-auto.svc.cluster.local`. Paste it into [network-automation-demo/inventory.yml](network-automation-demo/inventory.yml) under `ansible_host`, then run the AAP demo (Path A.4).
...
all:
  hosts:
    auto-hosts:
      ansible_host: rhel-10-manitoba-auto.headless.rhel-10-manitoba-auto.svc.cluster.local
      ansible_user: rhel
      ansible_port: 22
```
